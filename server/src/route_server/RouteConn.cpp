...
void CRouteConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        case CID_OTHER_ONLINE_USER_INFO:
            ...
            //route_server处理后，只是简单地记录一下每个msg_server上的用户数量和用户id：
            _HandleOnlineUserInfo( pPdu );
            break;
        case CID_OTHER_USER_STATUS_UPDATE:
            //这个消息是当某个msg_server上有用户上下线时，msg_server会给route_server发送自己最近的用户数量和在线用户id信息，
            //route_server的处理也就是更新下记录的该msg_server上的用户数量和用户id。
        ...
        case CID_BUDDY_LIST_REMOVE_SESSION_NOTIFY:
            //这几个消息都是往外广播消息，由于msg_server 可以配置多个，A给B发了一条消息，
            //必须广播在各个msg_server 才能知道B到底在哪个msg_server上
