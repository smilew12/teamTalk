...
namespace DB_PROXY {
    ...
    /**
     * 每一个新的socket不仅对应一个CBaseSocket对象，同时也对应一个连接对象CImConn
     * （可能会被具体化成对应的子类，如CProxyConn）。
     * 这些连接对象被放在另外一个全局map g_proxy_conn_map里面进行管理。
    */
void doLogin(CImPdu* pPdu/**登录参数请求包 */, uint32_t conn_uuid/*连接对象的id*/)
{
