...
private:
	...
	list<CDBConn*>	m_free_list;        //实际保存mysql连接的容器 连接池对象CDBPool用一个成员变量存储自己的若干个mysql连接：
...
private:
	...
	//存储mysql连接池，每个池子里面有多个mysql连接对象，配置文件中总共配置了主从两个数据库连接池
	//teamtalk_master,teamtalk_slave
