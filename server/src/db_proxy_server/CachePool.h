...
private:
	...
	//每一个缓存池对象CachePool的成员变量m_free_list中存储着若干个与redis的连接对象，具体是多少个，根据配置文件来配置
...
private:
	...
	//存储了配置文件配置的缓存池，key缓存池的名字，value缓存池CachePoll对象的指针
	//dbproxyserver.conf目前配置了如下几个redis缓存池：CacheInstances=unread,group_set,token,sync,group_member
