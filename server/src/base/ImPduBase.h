...
typedef struct {
    ...
    uint16_t	service_id;	  //
    uint16_t	command_id;	  //通过包头的command_id就知道该包是什么数据了
    uint16_t	seq_num;     // 包序号
    uint16_t    reversed;    // 保留
} PduHeader_t;


//IM  instant message 即时通讯  pdu  protocol data  unit 协议数据单元  通俗说就是一个包单位
...
protected:
    CSimpleBuffer	m_buf;          //包体
    PduHeader_t		m_pdu_header;   //包头
};
